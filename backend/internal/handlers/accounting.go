...
func CreateJournalEntry(c *gin.Context) {
	...
	c.JSON(http.StatusCreated, gin.H{"success": true, "data": gin.H{"id": jeID, "ref": req.Ref}})
}
...
func GetProfitLoss(c *gin.Context) {
	...
	database.DB.QueryRow(
		`SELECT COALESCE(SUM(jl.credit - jl.debit),0) FROM journal_lines jl
		 ...
		 WHERE je.company_id=$1 AND coa.type='revenue'`,
		companyID,
	).Scan(&totalRevenue)
	database.DB.QueryRow(
		`SELECT COALESCE(SUM(jl.debit - jl.credit),0) FROM journal_lines jl
		 JOIN journal_entries je ON je.id = jl.journal_id
		 JOIN chart_of_accounts coa ON coa.id = jl.account_id
		 WHERE je.company_id=$1 AND coa.type='expense'`,
		companyID,
	).Scan(&totalExpense)
	c.JSON(http.StatusOK, gin.H{
		"success":    true,
