...
func init() {
	RegisterApprovalFinalizer("sales", "so", func(docID, finalStatus string) {
		if finalStatus != "approved" {
			database.DB.Exec(`UPDATE sales_orders SET status=$1, updated_at=NOW() WHERE id=$2`, finalStatus, docID)
			return
		}
		var approverName string
		database.DB.QueryRow(
			`SELECT u.name FROM approval_actions aa
			 JOIN approval_requests ar ON ar.id = aa.request_id
			 JOIN users u ON u.id = aa.actor_id
			 WHERE ar.doc_id = $1 ORDER BY aa.acted_at DESC LIMIT 1`, docID,
		).Scan(&approverName)
		database.DB.Exec(`UPDATE sales_orders SET status='approved', approved_by=$1, updated_at=NOW() WHERE id=$2`, approverName, docID)
	})
}
...
func UpdateSOStatus(c *gin.Context) {
	...
	if req.Status == "approved" {
		var soNumber string
		var total int64
		database.DB.QueryRow(`SELECT so_number, total FROM sales_orders WHERE id=$1`, id).Scan(&soNumber, &total)
		if _, required, _ := SubmitForApproval(companyID, "sales", "so", id, soNumber, float64(total), userID); required {
			database.DB.Exec(`UPDATE sales_orders SET status='pending_approval', updated_at=NOW() WHERE id=$1`, id)
			database.WriteAuditLog(userID, "UPDATE", "sales_order", id, "SO diajukan untuk approval", c.ClientIP())
			c.JSON(http.StatusOK, gin.H{"success": true, "data": gin.H{"id": id, "status": "pending_approval"}})
			return
		}
