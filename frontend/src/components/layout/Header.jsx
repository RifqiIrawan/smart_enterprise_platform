...
import { pushApi, searchApi, rbacApi } from '@/api'
...
// Lets an admin "browse as" a different company than their own active session,
// without switching/reissuing their JWT (see authStore.setCompanyFilter). Only
// ever lists companies rbacApi.getCompanies() returns, i.e. companies the user
// actually has access to (primary or user_companies membership; all of them
// for superadmin) — nothing further to restrict client-side.
function CompanyFilterDropdown() {
  const { companyFilter, setCompanyFilter } = useAuthStore()
  const [open, setOpen] = useState(false)
  ...
  const loadCompanies = async () => {
    try {
      const res = await rbacApi.getCompanies()
  ...
  return (
    <div ref={ref} className="relative hidden md:block">
      <button
        onClick={() => { if (!open) loadCompanies(); setOpen(!open) }}
        title="Filter perusahaan yang ditampilkan"
      ...
      >
        ...
        <span className="max-w-[9rem] truncate">{companyFilter ? companyFilter.name : 'Semua Perusahaan Saya'}</span>
        <ChevronDown className="w-3 h-3" />
      </button>

      {open && (
        <div className="absolute right-0 top-full mt-2 w-64 rounded-2xl shadow-xl border z-50 overflow-hidden animate-scale-in" style={{ backgroundColor: 'var(--dropdown-bg)', borderColor: 'var(--border-color)' }}>
          <div className="px-3 pt-3 pb-1 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
            Filter Perusahaan
          </div>
          <ul className="max-h-72 overflow-y-auto pb-2">
            <li>
              <button
                onClick={() => { setCompanyFilter(null); setOpen(false) }}
                className={`w-full flex items-center justify-between px-3 py-2 text-sm hover:bg-slate-50 text-left transition-colors ${!companyFilter ? 'text-indigo-600 font-medium' : 'text-slate-700'}`}
              >
                Ikuti Perusahaan Saya
              </button>
            </li>
            {companies.map((co) => (
              <li key={co.id}>
                <button
                  onClick={() => { setCompanyFilter({ id: co.id, name: co.name }); setOpen(false) }}
                  className={`w-full flex items-center justify-between px-3 py-2 text-sm hover:bg-slate-50 text-left transition-colors ${companyFilter?.id === co.id ? 'text-indigo-600 font-medium' : 'text-slate-700'}`}
                >
                  <span className="truncate">{co.name}</span>
                  {co.current && <span className="text-[9px] text-slate-400 shrink-0 ml-2">Aktif</span>}
                </button>
              </li>
            ))}
          </ul>
