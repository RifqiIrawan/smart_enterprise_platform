...
import {
  ...
  Trash2, PackageCheck, Target, ArrowRight, RefreshCw,
...
function TabQuotation({ customerOptions, customers, productOptions }) {
  ...
  const canAdd = canDo('sales.quotation', 'add')
  const canEdit = canDo('sales.quotation', 'edit')
...
function TabRetur({ soList }) {
  const { canDo } = useAuthStore()
  const canAdd = canDo('sales.retur', 'add')
  const canEdit = canDo('sales.retur', 'edit')
...
function TabCRM() {
  const { canDo } = useAuthStore()
  const canAdd = canDo('sales.crm', 'add')
  const canEdit = canDo('sales.crm', 'edit')
...
const VALID_SALES_TABS = ['so', 'do', 'invoice', 'quotation', 'retur', 'crm', 'customer']

export default function Sales() {
  const { tab } = useParams()
  const navigate = useNavigate()
  const activeTab = VALID_SALES_TABS.includes(tab) ? tab : 'so'
  useEffect(() => {
    if (!VALID_SALES_TABS.includes(tab)) navigate('/sales/so', { replace: true })
  }, [tab])
  const { canDo } = useAuthStore()
  const menuKey = `sales.${activeTab}`
  ...
  const sectionTitle = {
    so: 'Sales Order', do: 'Delivery', invoice: 'Invoice', quotation: 'Quotation',
    retur: 'Retur', crm: 'CRM', customer: 'Customer',
  }[activeTab]
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Sales & CRM — {sectionTitle}</h1>
        ...
        {activeTab === 'so' && (
          <CardContent className="pt-4">
            <DataTable ref={soRef} fetchFn={salesApi.getSalesOrders} columns={soColumns}
              toolbar={canAdd && <Button size="sm" onClick={() => setShowSOModal(true)}><Plus className="w-4 h-4 mr-1" /> Buat SO</Button>}
              actions={(row) => (
                <div className="flex gap-1">
        ...
        {activeTab === 'do' && (
          <CardContent className="pt-4">
            <DataTable ref={doRef} fetchFn={salesApi.getDeliveryOrders} columns={doColumns}
              toolbar={canAdd && <Button size="sm" onClick={() => setShowDOModal(true)}><Plus className="w-4 h-4 mr-1" /> Buat DO</Button>}
              actions={(row) => (
                <div className="flex gap-1">
                  {canEdit && row.status === 'draft' && (
        ...
        {activeTab === 'invoice' && (
          <CardContent className="pt-4">
            <DataTable ref={invRef} fetchFn={salesApi.getInvoices} columns={invColumns}
              toolbar={canAdd && <Button size="sm" onClick={() => setShowInvModal(true)}><Plus className="w-4 h-4 mr-1" /> Buat Invoice</Button>}
            />
          </CardContent>
        )}
        ...
        {activeTab === 'customer' && (
          <CardContent className="pt-4">
            <DataTable ref={custRef} fetchFn={salesApi.getCustomers} columns={customerColumns}
              toolbar={canAdd && <Button size="sm" onClick={() => { setEditCustomer(null); setCustForm(emptyCustomer); setShowCustModal(true) }}><Plus className="w-4 h-4 mr-1" /> Tambah Customer</Button>}
              actions={(row) => (
                <div className="flex gap-1">
                  {canEdit && (
