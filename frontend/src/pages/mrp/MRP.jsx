...
import {
  PlayCircle, AlertTriangle, CheckCircle2, Clock,
  Plus, Trash2, Pencil, Package, Layers
} from 'lucide-react'
...
function MRPTab() {
  ...
  const canAdd = canDo('mrp.engine', 'add')
...
function JadwalTab() {
  const { canDo } = useAuthStore()
  const canAdd = canDo('mrp.jadwal', 'add')
  const canEdit = canDo('mrp.jadwal', 'edit')
  const canDelete = canDo('mrp.jadwal', 'delete')
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        {canAdd && <Button icon={Plus} onClick={() => setShowModal(true)}>Tambah Jadwal</Button>}
      </div>
...
function RoutingTab() {
  const { canDo } = useAuthStore()
  const canAdd = canDo('mrp.routing', 'add')
  const canEdit = canDo('mrp.routing', 'edit')
  const canDelete = canDo('mrp.routing', 'delete')
  const [showModal, setShowModal] = useState(false)
...
function LotTab() {
  const { canDo } = useAuthStore()
  const canAdd = canDo('mrp.lot', 'add')
  const canDelete = canDo('mrp.lot', 'delete')
  const [showModal, setShowModal] = useState(false)
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        {canAdd && <Button icon={Plus} onClick={() => setShowModal(true)}>Buat Lot Number</Button>}
      </div>
...
const VALID_TABS = ['engine', 'jadwal', 'routing', 'lot']
const SECTION_TITLE = {
  engine: 'MRP Engine', jadwal: 'Jadwal Produksi', routing: 'Routing', lot: 'Lot Tracking',
}

export default function MRP() {
  const { tab } = useParams()
  const navigate = useNavigate()
  const activeTab = VALID_TABS.includes(tab) ? tab : 'engine'
  useEffect(() => {
    if (!VALID_TABS.includes(tab)) navigate('/mrp/engine', { replace: true })
  }, [tab])
