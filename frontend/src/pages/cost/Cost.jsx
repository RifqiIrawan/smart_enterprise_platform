...
import { Calculator, Building2, Package, BarChart3, TrendingDown, TrendingUp, Plus, Trash2, RefreshCw, Save, AlertTriangle } from 'lucide-react'
  ...
  return (
    <div className="space-y-4">
      <DataTable
        ...
        columns={columns}
        toolbar={<button onClick={openNew} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 hover:from-indigo-600 hover:to-violet-700 text-white text-sm font-semibold shadow-md transition-all"><Plus className="w-3.5 h-3.5" />Tambah</button>}
        actions={(row) => (
          <div className="flex gap-1">
            <button onClick={() => openEdit(row)} className="px-2 py-1 rounded text-xs text-blue-600 hover:bg-blue-500/10 transition-colors">Edit</button>
            <button onClick={() => del(row.id)} className="px-2 py-1 rounded text-xs text-red-600 hover:bg-red-500/10 transition-colors"><Trash2 className="w-3 h-3" /></button>
          </div>
        )}
      />
...
export default function Cost() {
  ...
  const tabs = [
    { key: 'centers', label: 'Cost Center', icon: Building2 },
    { key: 'wo', label: 'Biaya WO', icon: Package },
    { key: 'std', label: 'Standard Cost', icon: Calculator },
    { key: 'laporan', label: 'Laporan & Analisis', icon: BarChart3 },
  ]
