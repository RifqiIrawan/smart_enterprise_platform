...
const VALID_TABS = ['fleet', 'fuel']
const SECTION_TITLE = { fleet: 'Armada', fuel: 'Log BBM' }

export default function SmartVehicle() {
  const { tab } = useParams()
  const navigate = useNavigate()
  const activeTab = VALID_TABS.includes(tab) ? tab : 'fleet'
  useEffect(() => {
    if (!VALID_TABS.includes(tab)) navigate('/vehicle/fleet', { replace: true })
  }, [tab])
  const { canDo } = useAuthStore()
  const canAdd = canDo(`vehicle.${activeTab}`, 'add')
        ...
        <Card>
          ...
          <CardContent>
            <DataTable
              ...
              defaultPageSize={10}
              toolbar={canAdd && <Button size="sm" icon={<Plus />} onClick={() => setShowModal(true)}>Catat BBM</Button>}
            />
          </CardContent>
        </Card>
      )}
