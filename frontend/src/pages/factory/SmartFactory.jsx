...
const VALID_FACTORY_TABS = ['overview', 'workorder', 'machines', 'bom', 'downtime', 'workcenters', 'routing', 'oee', 'scrap', 'capacity', 'report']

export default function SmartFactory() {
  const { tab } = useParams()
  const navigate = useNavigate()
  const activeTab = VALID_FACTORY_TABS.includes(tab) ? tab : 'overview'
  useEffect(() => {
    if (!VALID_FACTORY_TABS.includes(tab)) navigate('/factory/overview', { replace: true })
  }, [tab])
  const { canDo } = useAuthStore()
  const menuKey = `factory.${activeTab}`
  ...
  const sectionTitle = {
    overview: 'Overview', workorder: 'Work Order', machines: 'Mesin', bom: 'BOM', downtime: 'Downtime',
    workcenters: 'Work Center', routing: 'Routing', oee: 'OEE Real', scrap: 'Scrap/Rework',
    capacity: 'Kapasitas', report: 'Laporan',
  }[activeTab]
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          {canAdd && activeTab === 'workorder' && <Button icon={<Plus />} size="sm" onClick={() => setShowModal(true)}>Work Order Baru</Button>}
          {canAdd && activeTab === 'machines' && <Button icon={<Plus />} size="sm" onClick={() => { setEditMachine(null); setMachineForm(emptyMachine); setShowMachineModal(true) }}>Tambah Mesin</Button>}
          {canAdd && activeTab === 'bom' && <Button icon={<Plus />} size="sm" onClick={() => setShowBOMModal(true)}>Tambah BOM</Button>}
          {canAdd && activeTab === 'downtime' && <Button icon={<Plus />} size="sm" onClick={() => setShowDowntimeModal(true)}>Catat Downtime</Button>}
          {canAdd && activeTab === 'workcenters' && <Button icon={<Plus />} size="sm" onClick={() => setShowWCModal(true)}>Tambah Work Center</Button>}
          {canAdd && activeTab === 'routing' && <Button icon={<Plus />} size="sm" onClick={() => setShowRoutingModal(true)}>Tambah Routing</Button>}
          {canAdd && activeTab === 'scrap' && <Button icon={<Plus />} size="sm" onClick={() => setShowScrapModal(true)}>Catat Scrap/Rework</Button>}
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <DataTable
              ...
              defaultPageSize={10}
              toolbar={canAdd && <Button size="sm" icon={<Plus />} onClick={() => setShowModal(true)}>Tambah WO</Button>}
              actions={(row) => canEdit && (
                <button
        ...
        <Card>
          ...
          <CardContent>
            <DataTable
              ...
              defaultPageSize={10}
              toolbar={canAdd && <Button size="sm" icon={<Plus />} onClick={() => { setEditMachine(null); setMachineForm(emptyMachine); setShowMachineModal(true) }}>Tambah Mesin</Button>}
              actions={(row) => canEdit && (
        ...
        <Card>
          ...
          <CardContent>
            <DataTable
              ...
              defaultPageSize={10}
              toolbar={canAdd && <Button size="sm" icon={<Plus />} onClick={() => setShowBOMModal(true)}>Tambah BOM</Button>}
            />
          </CardContent>
        </Card>
      )}
        ...
        <Card>
          ...
          <CardContent>
            <DataTable
              ...
              defaultPageSize={10}
              toolbar={canAdd && <Button size="sm" icon={<Plus />} onClick={() => setShowDowntimeModal(true)}>Catat Downtime</Button>}
            />
          </CardContent>
        </Card>
      )}
