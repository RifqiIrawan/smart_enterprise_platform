...
export default function Marketplace() {
  ...
  useEffect(() => {
    if (!VALID_TABS.includes(tab)) navigate('/marketplace/overview', { replace: true })
  }, [tab])
  const { canDo } = useAuthStore()
  const canEditOrders = canDo('marketplace.orders', 'edit')
  const canEditListings = canDo('marketplace.listings', 'edit')
    ...
    <div className="p-6 space-y-5">
      ...
      {activeTab === 'orders' && (
        <DataTable
          ...
          actions={(row) => (canEditOrders && row.status === 'pending') ? (
