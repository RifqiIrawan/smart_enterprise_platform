...
function InspeksiTab() {
  ...
  const canAdd = canDo('qms.inspeksi', 'add')
  const canEdit = canDo('qms.inspeksi', 'edit')
  const canDelete = canDo('qms.inspeksi', 'delete')
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        {canAdd && <Button icon={Plus} onClick={() => { setForm(blank); setShowModal(true) }}>Tambah Inspeksi</Button>}
      </div>
...
function NCRTab() {
  const { canDo } = useAuthStore()
  const canAdd = canDo('qms.ncr', 'add')
  const canEdit = canDo('qms.ncr', 'edit')
  const canAddCapa = canDo('qms.capa', 'add')
  const canDelete = canDo('qms.ncr', 'delete')
  const [showModal, setShowModal] = useState(false)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex justify-end">
        {canAdd && <Button icon={Plus} onClick={() => { setForm(blank); setShowModal(true) }}>Buat NCR</Button>}
      </div>
...
function CAPATab() {
  const { canDo } = useAuthStore()
  const canEdit = canDo('qms.capa', 'edit')
  ...
  const columns = [
    ...
    { key: 'actions', label: '',
      render: (_, row) => (canEdit && row.status === 'open') ? (
...
function KalibrasiTab() {
  const { canDo } = useAuthStore()
  const canAdd = canDo('qms.kalibrasi', 'add')
  const canEdit = canDo('qms.kalibrasi', 'edit')
  const canDelete = canDo('qms.kalibrasi', 'delete')
  const [showModal, setShowModal] = useState(false)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex justify-end">
        {canAdd && <Button icon={Plus} onClick={() => { setForm(blank); setShowModal(true) }}>Tambah Alat Ukur</Button>}
      </div>
...
const VALID_QMS_TABS = ['inspeksi', 'ncr', 'capa', 'kalibrasi']
const QMS_SECTION_TITLE = { inspeksi: 'Inspeksi QC', ncr: 'NCR', capa: 'CAPA', kalibrasi: 'Kalibrasi' }

export default function QMS() {
  const { tab } = useParams()
  const navigate = useNavigate()
  const activeTab = VALID_QMS_TABS.includes(tab) ? tab : 'inspeksi'
  useEffect(() => {
    if (!VALID_QMS_TABS.includes(tab)) navigate('/qms/inspeksi', { replace: true })
  }, [tab])

  return (
    <div className="space-y-5">
      <h1 className="text-lg font-semibold text-slate-800">{QMS_SECTION_TITLE[activeTab]}</h1>
