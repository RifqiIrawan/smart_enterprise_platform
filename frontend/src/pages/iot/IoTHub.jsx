...
export default function IoTHub() {
  ...
  const activeTab = VALID_TABS.includes(tab) ? tab : 'live'
  useEffect(() => {
    if (!VALID_TABS.includes(tab)) navigate('/iot-hub/live', { replace: true })
  }, [tab])
  const { canDo } = useAuthStore()
  const canAddDevice = canDo('iot.devices', 'add')
  const canEditDevice = canDo('iot.devices', 'edit')
  const canAddRule = canDo('iot.alerts', 'add')
  const canDeleteDevice = canDo('iot.devices', 'delete')
  const canDeleteRule = canDo('iot.alerts', 'delete')
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>{TABS.find(t => t.id === activeTab)?.label}</h1>
        ...
        {activeTab === 'devices' && canAddDevice && (
