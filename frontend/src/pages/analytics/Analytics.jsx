          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            {results && (
              <>
                ...
                <Button variant="secondary" size="sm" icon={<Calendar className="w-3.5 h-3.5" />} onClick={() => setShowScheduleModal(true)}>Jadwalkan</Button>
              </>
            )}
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" icon={<Plus />} onClick={() => setShowScheduleModal(true)}>Tambah Jadwal</Button>
          </div>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
...
export default function Analytics() {
  ...
  const tabs = [
    ...
    { id: 'hr', label: 'HRIS' },
    { id: 'module', label: 'Kesehatan Modul' },
    { id: 'comparison', label: 'Perbandingan', ai: false },
    { id: 'forecast', label: 'Demand Forecast', ai: true },
    { id: 'predictive', label: 'Predictive Maint.', ai: true },
    { id: 'anomalies', label: 'Anomali', ai: true },
    { id: 'price', label: 'Rekomendasi Harga', ai: true },
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg flex-wrap">
        {tabs.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            className={`flex items-center gap-1 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === t.id ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
            {t.ai && <Brain className="w-3 h-3 text-purple-500" />}
            {t.label}
          </button>
        ))}
      </div>
